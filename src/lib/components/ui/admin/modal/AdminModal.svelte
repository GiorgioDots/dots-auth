...
{#if open}
	<div  class="absolute top-0 left-0 z-20 h-full w-full">
		<div
			class="bg-background-100 absolute top-0 left-0 h-full w-full opacity-60"
			onclick={() => onclose?.()}
...
<style>
	@reference '../../../../../app.css';
	.modal {
		@apply bg-background-300 rounded-default absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transform p-4;
	}
</style>
